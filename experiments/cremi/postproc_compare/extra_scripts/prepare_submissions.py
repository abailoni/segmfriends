...
def run_block_segm(project_directory, exp_name):
    scores_path = os.path.join(project_directory, exp_name, "scores/combined")
    ...
    for agglotype in results_collected:
        for sample in results_collected[agglotype]:
            first_config = results_collected[agglotype][sample]['0']
            ...
            filename = first_config["score_filename"]
            out_segm_dir = os.path.join(scores_path, "../../out_segms/combined")
            check_dir_and_create(out_segm_dir)
            out_segm_path = os.path.join(out_segm_dir, filename.replace(".yml", ".h5"))
