...
from pathutils import get_home_dir
...
project_dir = os.path.join("./scores")
...
for item in os.listdir(scores_path):
    if os.path.isfile(os.path.join(scores_path, item)):
        ...
        new_table_entrance = ["{}".format(spl) for spl in filename.replace(".yml", "").split("__")]
        if LATEX_OUTPUT:
            new_table_entrance = [itm.replace("_", "\_") for itm in new_table_entrance]
